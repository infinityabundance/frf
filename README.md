...
- **Minimization is executable**: `frf court minimize` routes the residual's
  ...
  (`frf-reduction-v3`, which binds the external minimizer's semantic +
  implementation identities when one reduced) and the final reproducer
  court-verified. v0.1.30 reducers: text fixtures at line granularity
  (binary fixtures refused). Domain reducers (argv, environment variables,
...
- **Residual trajectories are executable over five axes**: the repeat
  ...
  (`frf-series-v2`): an append is a new immutable node of the experiment's
...
- **Bundles are portable in both containers and replayable alone**: the
  ...
  read-only and never executes. Witness statements stay future work until
  that protocol object exists.
