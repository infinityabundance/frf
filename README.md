...
| `frf residual dispose ID --disposition D --reason "..."` | appends an immutable, content-addressed disposition EVENT to `residuals/<id>.events/` (`fixed \| nonreproduced \| stabilized \| intentional \| environmental \| oracle_version \| harness \| unknown`); a one-line reason is mandatory, `open` is not settable, and the evidence-bearing dispositions each require their verified evidence edge: `fixed` requires `--resolution-run` (a court run that reran the same question under a compatible envelope, shows the residual no longer reproduces, AND executed a CHANGED candidate artifact — a pass on the same candidate is a non-reproduction, not a fix), `nonreproduced` requires `--observation-run` (one pass on the SAME candidate — still blocks claims), `stabilized` requires `--trajectory` + `--consecutive-passes` (repeated passes on the SAME candidate, trajectory-verified — still blocks claims). A disposition is not evidence. Events are hash-chained: each carries its own `event_id` (SHA-256 of its content), its `parent_event_id`, and its `evidence_refs`; the append is a COMPARE-AND-SWAP against the chain's last event (a concurrent writer that appended first is a conflict, and the CLI re-reads and retries). The observation file is never rewritten; the current disposition is the projection of the last event |
...
| `frf bundle export RECEIPT_ID [--output PATH] [--single]` | exports a receipt's portable closure — manifest + receipt + captures (+ the externally served axes' comparator request/response/invocation/result evidence + the normalizer/capture-adapter invocation evidence and any external-minimizer refusal evidence) + content-addressed objects (the executed artifacts AND the comparator/normalizer/adapter/minimizer instrumentation, walked via the capture's typed `evidence_refs`) + the admitted authority record + residuals + disposition-event chains (+ the compiled claim when present, WITH its capability evidence — the challenge records and mutant runs a sensitivity-backed claim was compiled under, the witness statements + preserved documents an independently-witnessed claim names, and the claim's COMMITTED KNOWLEDGE UNIVERSE — every authority/series/reduction/receipt/run the absence search depended on, carried generically from the snapshot's typed objects, so a verifier can re-derive each committed content address from the bundle alone), sealed read-only, only after the receipt verifies against the source tree. The manifest (frf-bundle-v3) declares its own CONTAINER: a `directory` tree (default, `bundles/<receipt-id>.frf`), or `--single` — ONE deterministic tar archive with the manifest inside: fixed metadata, entries in path order, so two exports of the same receipt are byte-identical |
| `frf bundle verify BUNDLE.frf` | verifies a bundle against ITSELF (either container — a directory, or a single-file archive, auto-detected; BOTH are materialized into a private safe tree before a single byte is read — the directory form is copied by the SAME confinement walk as the archive extractor: symlinks and hard links refused (a link could resolve outside the bundle), escapes refused, 10 000-entry / 1 GiB caps enforced; the manifest is parsed strictly (I-JSON, closed schema, every identifier validated, the inventory-role vocabulary closed); and the exact inventory is proven: every file must exist and hash to its recorded digest, objects named by their digest. Only then does the receipt verify against the bundled evidence alone — no original source tree, no exporting FRF installation). Verification never executes anything |
...
| execution | What was actually executed (a run: identity + capture + produced trees); the capture identity is the FRF/RUN/v2 composition of the observation identity (FRF/OBSERVATION/v1 — what was observed) and the execution identity (FRF/EXECUTION/v1 — under exactly what machinery/contract it was observed). v15: the capture binds the harness events recorded during the run's observation (the bound-firing records that completed as valid side outcomes) | frf-capture-v15 · FRF/RUN/v2 | active |
| harness-event | Evidentiary overflow record: a declared bound the harness ENFORCED during an observation attempt (stream-overflow | timeout | rlimit | produced-overflow), content-addressed under harness/<id>.json — the run is still refused (fail-closed, never truncated), but the refusal is itself provable evidence. A bound that refuses the run (stream/timeout/produced overflow) leaves no capture to bind to: those events are court-scoped refusal evidence, named by the refusal message; a bound that completes as a valid side outcome (the CPU limit's SIGXCPU) is cited by the run's capture (frf-capture-v15) and travels with the run's bundle | frf-harness-event-v1 · FRF/HARNESS-EVENT/v1 | active |
| execution-attempt | Refused observation attempt made first-class: when a court's observation attempt ends in a harness refusal (stream overflow, timeout, produced-tree overflow) there is no successful run to become the durable graph root of that attempt — the refusal itself is the observation about the attempt. A content-addressed immutable record under attempts/<id>.json binding the declared court + its semantic identity, the bound artifacts, the fixture, argv, the environment digest, the execution contract (profile + capture bounds as enforced), the side that refused, the recorded harness-event ids, and the refusal reason. The identity rederives on every read and every cited harness event is itself verified — a refusal is as portable as the observation that would have been captured. The `refused` arm of the conceptual ExecutionAttempt sum (completed attempts ARE runs); the error message is no longer the only surface through which a failed observation attempt is named | frf-execution-attempt-v1 · FRF/EXECUTION-ATTEMPT/v1 | active |
...
| high-assurance | Independently witnessed + every premise observed under a profile providing the REQUIRED CAPABILITY SET (the reference contract: exact_capture_contract, sealed_executable_image, native_runtime_closure_bound) under the reference capture bounds — protocol constants no FRF_EXEC_* override can redefine. Every admitted profile provides the set (v1 exactly; v2/v3/OCI as supersets) — assurance is orthogonal capabilities, never a profile-name equality; the claim records required_capabilities | active |
...
| frf-exec-linux-v3 | The I/O-CLOSED profile (0.1.65): on top of the reference setrlimit layer, the side's world is CLOSED before it runs a single instruction — the FILESYSTEM closure (Landlock, Linux >= 5.13): read/execute only the content-addressed objects directory, the execution machinery (the interpreter chain + its own native closure, the artifact's native runtime closure, the declared execution-context artifacts), and the declared randomness channel; the produced-output staging directory is the ONLY writable surface (absent = none at all); and the AMBIENT-CHANNEL closure (seccomp BPF): no network, no Unix sockets, no shared memory, no ptrace, no cross-process memory. A side that violates a closure fails visibly and the court OBSERVES the denial. The side executes its VERIFIED SNAPSHOT PATH (the sealed-memfd mechanism is incompatible with a Landlock closure: the kernel cannot bind access rules to anonymous-inode memfds — proven empirically); the OCI profile remains the mechanism that closes both the path race and the ambient-environment race. Replay rebuilds the closure from the capture's recorded machinery. Requires Landlock; without it the profile REFUSES to run — a declared profile is enforced, never approximated | active |
| frf-exec-oci | The OCI container profile (0.1.62): each side runs INSIDE a container from a digest-pinned OCI image (the reference must carry its manifest digest — a mutable tag is refused), with no network, the declared environment, and the working directory bind-mounted at its own absolute path so recorded argv paths resolve inside the container. The image IS the complete execution machinery — the whole root filesystem (interpreter, shared libraries, loader configuration, certificates) is bound by the digest in the execution identity. Requires a container runtime (podman or docker) and the exact image present; a missing runtime or image REFUSES the run — never a silent substitution. Extensions are harness-side instrumentation and run on the host. An OCI observation is a different execution contract from a v1 one; both qualify for high-assurance when they provide the required capability set (the reference contract — spec/execution-profile.md § assurance capabilities) | active |
...
|---|---|---|
| regression | `cargo test` | the invariant bank: every verb, every rejection path, reason-gate, re-disposition, id/path-safety boundary, fail-closed envelope enforcement, object-store corruption refusal, timeout kill, the court-challenge negative controls (spec/challenge.md), a zero-residual positive control, and the CANONICAL-LOADER GATE (tests/loader_gate.rs): every `serde_json::from_slice` of an evidence struct in the engine's source must live inside exactly one of the three canonical loaders — `Store::parse_evidence`, `ext::parse_canonical_response`, `canon::parse_strict` — so a direct typed projection can never be parsed before the document's canonical identity is established (a new direct call site is a build failure, and the gate's self-test proves it catches a planted violation) |
...
| empirical | `make experiment` · `cargo xtask experiment golden/work/experiment.json` · `cargo xtask external-experiment golden/work/external-experiment.json` · `cargo xtask external-experiment-v2 golden/work/external-experiment-v2.json` · `cargo xtask external-experiment-v3 golden/work/external-experiment-v3.json` · `cargo xtask external-experiment-v4 golden/work/external-experiment-v4.json` | the empirical program (spec/empirical-program.md): seeded mutations over the cross-domain corpus (CLI, filesystem tree, byte/wire, structured state, timing — 7 seeded defects + 5 clean controls), measured against conventional suites — defect discovery (every seed detected), specificity (zero false positives), claim inflation (no claim covers a seeded-defect axis; every clean claim is bounded), minimization cost (ddmin attempts + reduction), replay stability (36/36 byte-identical replays), and evidence overhead (FRF bytes vs a pass/fail baseline). The EXTERNAL program drives the same measurements over REAL historical defects — Apple's `goto fail` (CVE-2014-1266), bash's Shellshock (CVE-2014-6271), OpenSSL's Heartbleed (CVE-2014-0160), Log4j's Log4Shell (CVE-2021-44228), the Mars Climate Orbiter unit mismatch, and the two-digit-year Y2K bug — each with a fixed reference, the buggy historical candidate, a distinct clean control, and a challenge that proves the court can SEE its domain's defect class (external mutation providers for the wire/state domains). The v2 program drives the SAME corpus through the trajectory axes: version ladders (buggy revision → clean revision; the defect lineage must classify `boundary-localized` cessation), environment matrices (the defect at three deterministic TZ/LANG coordinates; the trajectory must be `persistent`/`stable`), and authority transitions (the historical vulnerable program IS the pre-fix oracle, admitted alongside the fixed one — the buggy candidate's defect becomes observable exactly when the oracle was fixed, `boundary-localized` onset; the clean candidate's stricter behavior ceases, `boundary-localized` cessation; a diagnostic-wording lineage that persists is measured as a note, not a failure). The v3 program replaces the RECONSTRUCTED reproducers with the ACTUAL upstream releases — bash 4.3.0→4.3.30 (Shellshock), OpenSSL 1.0.1f→1.0.1g (Heartbleed), Log4j 2.14.1→2.17.1 (Log4Shell) — built from pinned sources by digest- and version-pinned reproducible build recipes (`external-corpus/v3/build/build-all.sh`, provenance in build-manifest.json), with the same ladder/env/authority trajectory gates PLUS a clean control (the vulnerable side without the trigger must produce ZERO residuals). The v4 program is the full comparative MEASUREMENT STUDY over the v3 corpus — the metric table of the empirical-program review — executing the conventional baselines BARE on the same bytes (golden pinned to the fixed release; golden pinned to the vulnerable release, which correctly reports every historical fix as a behavior CHANGE from its recorded oracle — a snapshot test detects change, it does not claim the old behavior was correct; differential; a PROJECTED unit-suite baseline — no independent unit suite is executed, and the row says so) next to FRF court / +challenge / +trajectory / +minimization, and measuring defects detected, false positives, version and environment boundaries, nondeterminism (the 5× repeat probe must collapse to one content-addressed run), claim inflation prevented (the buggy receipt's claim never covers a defect axis; the fixed-side claim's surface GROWS to the full declared scope across the boundary; the clean claim is REFUSED by the cross-run universe blocker — an open divergence about the same surface blocks the claim even from a passing run), minimum reproducer size (honest one-minimal at line granularity for the single-line fixtures), replay stability, evidence storage amplification (evidence records vs raw output vs a pass/fail baseline), a single-host runtime-overhead PILOT (one bare execution vs the FRF per-run cost, millisecond smoke — not yet a performance study), localization execution count (an operation-count proxy, not measured effort), and the investigation surface (the evidence-object inventory — a useful proxy, not human investigation cost). Exits non-zero if any measurement violates the standards
...
- **Corpus admission and independent witness maps** — v0 proves the kernel on
  ...
  Log4j 2.14.1/2.17.1) built from pinned sources by digest- and version-pinned reproducible container
...
- **Minimization is executable**: `frf court minimize` routes the residual's
  ...
  (`frf-reduction-v5`, which binds the external minimizer's semantic +
